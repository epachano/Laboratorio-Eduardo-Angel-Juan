...
class AVLTree:
    ...
    def graph (self, root, counter, tree):
        tree.node(f'{counter}', f'{root.name}')
        
        if root.left:
            temp_counter = counter
            tree, counter = self.graph(root.left, counter+1, tree)
            tree.edge(f'{counter}', f'{temp_counter}', '')
    ...
    def find_rec_path(self, root, key, path):
        if root is None:
            return False
        ...
        if (root.left and self.find_rec_path(root.left, key, path)) or \
           (root.right and self.find_rec_path(root.right, key, path)):
            return True
    ...
    def path (self, start_name, end_name):
        path = []
        start = self.search(name=start_name)
        end = self.search(name=end_name)
